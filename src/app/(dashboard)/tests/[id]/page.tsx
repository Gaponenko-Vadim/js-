...
export default function TestPage() {
  ...
  }, [testId, testStarted, userAnswers, currentQuestionIndex, selectedAnswer, testMode, timeLeft, endTime, showResults]);
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = () => {
      if (testId) {
        sessionStorage.setItem(`test_${testId}_reloading`, 'true');
      }
    };
    ...
    return () => {
      ...
      if (testId && testMode === 'exam' && testStarted && !showResults) {
        const isReloading = sessionStorage.getItem(`test_${testId}_reloading`) === 'true';
  ...
  }, [testId, testMode, testStarted, showResults]);

  // Очищаем флаг перезагрузки после восстановления
  useEffect(() => {
    if (testStarted && testMode === 'exam' && testId) {
      const reloadingFlag = sessionStorage.getItem(`test_${testId}_reloading`);
      if (reloadingFlag === 'true') {
        sessionStorage.removeItem(`test_${testId}_reloading`);
      }
    }
  }, [testStarted, testMode, testId]);
  ...
  const handleBackToTests = () => {
    ...
    // Для learning прогресс остается в sessionStorage

    // Перенаправляем на страницу с тестами категории
    // Используем параметр category из URL, если он есть
    if (categoryParam) {
      router.push(`/tests?category=${categoryParam}`);
    } else {
      // Fallback: используем первую категорию из данных теста
  ...
  const submitTest = async (answers: number[]) => {
    ...
    // Сохраняем результаты только в режиме экзамена
    if (testMode === 'exam') {
      try {
        ...
        <div className={styles.restoreDialog}>
          <div className={styles.restoreDialogContent}>
            ...
            <p className={styles.restoreDialogText}>
              У вас есть незавершённое прохождение теста <strong>{test.title}</strong> в режиме обучения
            </p>
