...
**User Stories (пользовательские истории)** работают точно так же. Это краткие карточки желаний пользователя, которые запускают разговор между командой и Product Owner'ом (владельцем продукта). Не подробная спецификация, а приглашение к беседе.
...
**User Story (пользовательская история)** — краткое описание функциональности с точки зрения пользователя.

Стандартный формат:
\`\`\`
As a [Role - роль]
I want [Feature/Goal - функция/цель]
So that [Benefit/Value - выгода/ценность]
\`\`\`
...
|-----------|----------|--------|
| **Role (роль)** | Кто нуждается в функции | Customer, Admin, Guest User |
| **Goal (цель)** | Что хочет сделать пользователь | save items to wishlist |
| **Benefit (выгода)** | Зачем, какая ценность | purchase them later conveniently |

### User Story vs Use Case vs Task

| Аспект | User Story | Use Case (сценарий использования) | Task (задача) |
|--------|------------|----------|------|
| **Уровень** | Высокий (бизнес-ценность) | Детальный (шаги) | Технический (реализация) |
| **Формат** | As-Want-SoThat | Structured document (структурированный документ) | Technical work item (техническая задача) |
...
\`\`\`
As a CUSTOMER (покупатель)
I want to SAVE ITEMS TO A WISHLIST (сохранять товары в избранное)
So that I can PURCHASE THEM LATER when I have budget (покупать их позже, когда будет бюджет)
\`\`\`

### Почему именно этот формат?

**As a [Role]** — фокусирует на **КТО** нуждается в функции:
- Помогает понять контекст и permissions (разрешения)
- Разные роли → разные потребности
- Можно создавать Personas (персоны) для ролей

**I want [Goal]** — описывает **ЧТО** хочет пользователь:
- Описывает action (действие) или capability (возможность)
- НЕ технические детали (не "хочу REST endpoint")
- User-facing функциональность (функциональность, видимая пользователю)
...
✅ **Хорошо**:
\`\`\`
As a shopper (покупатель)
I want to filter products by price range (фильтровать товары по диапазону цен)
So that I can find items within my budget quickly (быстро находить товары в рамках бюджета)
\`\`\`

✅ **Хорошо**:
\`\`\`
As a content creator (создатель контента)
I want to schedule posts for future publication (планировать посты на будущее)
So that I can maintain consistent posting without manual work (поддерживать регулярные публикации без ручной работы)
\`\`\`
...
✅ **Правильно**:
- Story A: "Add item to cart (добавить товар в корзину)"
- Story B: "Remove item from cart (удалить товар из корзины)"
(Можно реализовать в любом порядке)

❌ **Неправильно**:
- Story A: "Create database table for users (создать таблицу БД для пользователей)"
- Story B: "Implement user registration (реализовать регистрацию)"
(B зависит от A — нарушение независимости)

**Как исправить**: Объединить зависимые stories или изменить разбиение на Vertical Slices (вертикальные срезы).
...
> 💡 Technical Debt items (технический долг) могут быть исключением, но их нужно минимизировать.

### E — Estimable (Оцениваемая)

Команда может оценить effort (усилия) (Story Points или время).

✅ **Оцениваемая**: Команда понимает что делать, может оценить.

❌ **Неоцениваемая**: Слишком неопределенная ("Research новой технологии") → Нужен **Spike (исследование)**.
...
❌ **Большая**: Epic ("Complete checkout experience") → Нужно разбить на smaller stories (меньшие истории).

### T — Testable (Тестируемая)

Есть четкие критерии для проверки выполнения.

✅ **Тестируемая**: Есть Acceptance Criteria (критерии приемки):
\`\`\`
...
**Acceptance Criteria (критерии приемки)** — конкретные, проверяемые условия, при которых User Story считается завершенной.

> AC отвечают на вопрос: "Как я узнаю, что это сделано?"

### Формат: Given-When-Then (Gherkin)

Структура из BDD (Behavior-Driven Development - разработка через поведение):
...
AC2: Увеличение quantity (количества)
...
AC3: Out of stock (нет в наличии)
...
| Aspect (аспект) | Acceptance Criteria | Definition of Done (определение готовности) |
|--------|--------------------|--------------------|
| **Scope (область)** | Специфичны для каждой Story | Общие для ВСЕХ stories |
...
\`\`\`
Theme (тема): E-commerce Platform
  └─ Epic (эпик): Shopping Experience
      ├─ User Story: Browse Products
      │   ├─ Task (задача): Create product catalog API
...
**Epic (эпик)** — User Story, которая слишком велика для одного спринта (обычно >1 спринта работы).

**Пример Epic**: "As customer, I want complete checkout experience"

**Разбивается на Stories**:
1. "Review cart items (просмотр товаров в корзине)"
2. "Enter shipping address (ввод адреса доставки)"
3. "Select payment method (выбор способа оплаты)"
4. "Apply discount code (применение скидки)"
5. "Confirm and place order (подтверждение заказа)"

**Когда использовать Epic**:
- Roadmap planning (планирование дорожной карты) (высокий уровень)
- Группировка связанных stories
- Tracking прогресса большой функциональности

### Task — Техническая декомпозиция Story

**Task (задача)** — конкретная техническая работа для реализации Story.
...
- Task — technical (техническая), Story — user-facing (видна пользователю)
- Task estimated (оценена) в hours (часах), Story — в Story Points
- Task не видны stakeholders (заинтересованным сторонам)
...
**Story Points (очки истории)** — относительная единица измерения сложности/effort Story.
...
| **2** | Очень просто | Добавить validation (валидацию) на поле |
| **3** | Просто | Создать простую форму |
| **5** | Средняя сложность | CRUD для нового entity (сущности) |
| **8** | Сложно | Интеграция с third-party API |
| **13** | Очень сложно | Complex reporting feature (сложная функция отчетов) |
| **21** | Слишком большая | → Разбить на smaller stories |

> ⚠️ Story > 13 points обычно признак что нужен split (разбиение).

### Planning Poker — Техника оценки

**Planning Poker (покер планирования)** — техника коллективной оценки User Stories.
...
6. Повторять пока не достигнут consensus (консенсус)

**Преимущества**:
- Учитывает мнения всех членов команды
- Предотвращает anchoring bias (эффект привязки) (влияние первого мнения)
- Стимулирует обсуждение неопределенностей
- Вовлекает всю команду

### Velocity — Скорость команды

**Velocity (скорость)** = сумма Story Points завершенных stories за спринт.

**Пример**:
- Sprint 1: 23 points completed
- Sprint 2: 27 points completed
- Sprint 3: 25 points completed
- **Average Velocity (средняя скорость)**: ~25 points/sprint

Используется для **capacity planning (планирования мощности)** следующих спринтов.
...
**Definition of Done (DoD - определение готовности)** — набор общих критериев качества для КАЖДОЙ Story.

**Пример DoD**:
- ✅ Code написан и соответствует coding standards (стандартам кодирования)
- ✅ Unit tests написаны (>80% coverage - покрытие)
- ✅ Integration tests passed (интеграционные тесты пройдены)
- ✅ Code review пройден (минимум 1 reviewer - проверяющий)
- ✅ Documentation обновлена
- ✅ Deployed to staging environment (развернуто на staging окружении)
- ✅ QA testing passed
- ✅ Product Owner принял Story
- ✅ No critical bugs (нет критических багов)
...
**Definition of Ready (DoR - определение готовности)** — checklist (чек-лист) готовности Story к включению в спринт.

**Пример DoR**:
- ✅ Story имеет четкое описание в формате As-Want-SoThat
- ✅ Acceptance Criteria определены (Given-When-Then)
- ✅ Dependencies identified (зависимости определены) и resolved (решены)
- ✅ Story estimable (оцениваема) (команда может оценить)
- ✅ Story size <= спринт capacity (не Epic)
- ✅ UI mockups (макеты) готовы (если применимо)
- ✅ Stakeholders доступны для вопросов
- ✅ Нет blockers (блокеров)

**Зачем нужен DoR**:
- Предотвращает неготовые stories в спринте
- Снижает blockers во время разработки
- Уменьшает uncertainty и waste (потери)
...
**Vertical Slice (вертикальный срез)** проходит через все слои архитектуры:
...
- Можно тестировать integration (интеграцию)
- Earlier feedback (ранняя обратная связь)

### Horizontal Slice — Один слой (❌ Антипаттерн в Agile)

❌ **Пример Horizontal Slice**:
- Story 1: "Create all database tables (создать все таблицы БД)"
- Story 2: "Build all API endpoints (создать все API endpoints)"
- Story 3: "Build all UI screens (создать все UI экраны)"

**Проблемы**:
- Ничего не работает end-to-end до завершения всех stories
- Нельзя демонстрировать ценность
- Высокий integration risk (риск интеграции)
- Поздний feedback
...
**Spike (исследование)** — time-boxed (ограниченное по времени) исследование для снижения неопределенности.
...
**1. Technical Spike (техническое исследование)** — исследование технологии:
\`\`\`
...
**2. Functional Spike (функциональное исследование)** — исследование требований:
\`\`\`
...
- ✅ Исследовать feasibility (выполнимость) решения
- ✅ Изучить новый domain (область)
...
**Three Cs (три C)** (Ron Jeffries) — сущность User Story.
...
- **Placeholder (заполнитель)** для беседы
- Достаточно информации чтобы запомнить и приоритизировать

### 2. Conversation — Разговор

Детали обсуждаются между PO и командой **face-to-face (лицом к лицу)**.

- НЕ все заранее документировано (в отличие от Waterfall)
- Ongoing dialogue (постоянный диалог) во время разработки
...
> 💡 **Главная идея**: User Story ≠ requirements document (документ требований), это **promise of conversation (обещание разговора)**.
...
### Story Splitting Patterns (паттерны разбиения)

**1. Workflow Steps (шаги рабочего процесса)**

Epic: "Search products"
→ Split (разбиение):
- "Basic keyword search (базовый поиск по ключевым словам)"
- "Filter results by category (фильтрация по категории)"
- "Sort results by price (сортировка по цене)"
- "Advanced filters (расширенные фильтры) (brand, rating)"

**2. Business Rule Variations (вариации бизнес-правил)**
...
**3. CRUD Operations (CRUD операции)**
...
**4. Data Variations (вариации данных)**
...
**5. Simple/Complex versions (простая/сложная версии)**

Epic: "Product search"
→ Split:
- "Simple text search" (MVP)
- "Autocomplete suggestions" (enhancement - улучшение)
- "Fuzzy matching (нечеткое совпадение)" (advanced - продвинутая)
...
Метафора: **shortcuts (обходные пути) в коде**, отложенный рефакторинг.

**Примеры**:
- Quick & dirty решение вместо правильного
- Отсутствие unit tests
- Hardcoded values (жестко закодированные значения) вместо configuration
- Устаревшие dependencies (зависимости)
- Дублирование кода
...
**2. Tracking (отслеживание)**:
- Tech Debt backlog
- Регулярный review
- Оценка impact (влияния) и effort
...
**Product Backlog (бэклог продукта)** — приоритизированный список всех User Stories, требований, улучшений, багфиксов.

**Характеристики**:
- **Prioritized (приоритизированный)**: Top = most valuable/urgent
- **Dynamic (динамичный)**: Постоянно evolving (развивается)
- **Emergent (появляющийся)**: Детали добавляются со временем
...
**Product Backlog Refinement (Grooming - уточнение бэклога)**:
- Регулярная активность (обычно mid-sprint)
- Уточнение stories
- Splitting больших stories
- Re-prioritization (перерасстановка приоритетов)
- Estimation (оценка)
...
**Sprint Backlog (бэклог спринта)** = Selected Stories + Tasks + Sprint Goal (цель спринта)

**Формируется на Sprint Planning (планировании спринта)**:
1. Команда выбирает top stories из Product Backlog
2. Stories которые можно завершить за спринт (based on Velocity)
3. Decomposition (декомпозиция) stories на Tasks

**Owned by Development Team (команда разработки)** (не PO).

**Можно изменять**:
- ✅ Добавлять tasks
- ✅ Уточнять детали
- ❌ Добавлять новые stories без agreement (согласования)
...
**Преимущества**:
- Фокус на главном
- Управление scope (границ)
- Объяснение stakeholders

### Альтернативы MoSCoW

- **Kano Model (модель Кано)**: классификация по customer satisfaction (удовлетворенности клиента)
- **Value vs Effort Matrix (матрица ценность vs усилия)**: 2x2 матрица приоритизации
- **WSJF** (Weighted Shortest Job First - взвешенный кратчайшая работа первой): Cost of Delay (стоимость задержки) / Duration (длительность)
...
**Story Mapping (карта историй)** (Jeff Patton) — 2D карта организации User Stories.

**Структура**:
- **Горизонталь** (top row): User Activities (активности пользователя) (user journey steps - шаги пути пользователя)
...
**Horizontal Slices (Releases - релизы)**:
...
- ✅ Gap analysis (анализ пробелов) (пропущенные stories)
- ✅ Release planning (какие stories в какой release)
- ✅ Shared understanding (общее понимание) команды и stakeholders
...
**Persona (персона)** — фикциональный, но реалистичный персонаж, представляющий сегмент пользователей.
...
Имя: Sarah Johnson
Возраст: 35
Профессия: Busy working mom (занятая работающая мама)
Цели: Quick online grocery shopping without stress
Фрустрации: Complicated checkouts, long delivery times
Поведение: Shops on mobile during commute (покупки на мобильном по дороге на работу)
...
> 💡 Personas основаны на User Research (исследовании пользователей) (интервью, аналитика, surveys).
...
**MVP (Minimal Viable Product - минимальный жизнеспособный продукт)** — минимальный набор User Stories, достаточный для:
- Решения core проблемы пользователей
- Получения validated learning (подтвержденного обучения)
...
**Build → Measure → Learn → Iterate (строить → измерять → учиться → итерировать)**

1. **Build** MVP
2. **Measure** metrics (метрики) (conversions, usage)
3. **Learn** insights (озарения) (что работает, что нет)
4. **Iterate** на основе feedback

### Антипаттерны MVP

❌ **Gold Plating (золочение)**: Добавление "nice to have" фич в MVP
❌ **Scope Creep (расползание границ)**: Постепенное раздувание MVP
❌ **Perfect MVP**: Попытка сделать всё идеально с первого раза

> 🎯 MVP должен быть "embarrassingly simple (смущающе простым)" — если не стыдно показывать, возможно он не minimal.
...
**BDD (Behavior-Driven Development - разработка через поведение)** (Dan North) — подход к разработке через описание поведения системы.

**Ключевая идея**: Acceptance Criteria User Stories пишутся в Given-When-Then формате (Gherkin), затем превращаются в automated tests (автоматизированные тесты).
...
**Преимущества BDD**:
- ✅ Living Documentation (живая документация) (AC = tests = docs)
- ✅ Collaboration (сотрудничество) (PO, Dev, QA пишут AC вместе)
- ✅ TDD на уровне behavior
- ✅ Executable specifications (исполняемые спецификации)
...
**Stabilization (стабилизация)**: Velocity stabilizes через 3-5 спринтов.
...
80 |         ●
   |        ●
60 |       ●
   |      ●  (Actual completed - фактически завершено)
40 |     ●
   |    ●
20 |   ●
   |  ●
...
**Преимущество**: Видно добавление новых stories в backlog (scope changes - изменения границ).
...
- Bottlenecks (узкие места) (если In Progress растет)
- Flow эффективность
...
1. "Create user profile (создать профиль)" (5 SP)
2. "View user profile (просмотр профиля)" (3 SP)
3. "Edit profile information (редактирование)" (5 SP)
4. "Upload profile picture (загрузка фото)" (8 SP)
5. "Delete account (удаление аккаунта)" (5 SP)
...
- **13 SP voter**: "Нужна обработка webhook'ов, retry logic (логика повтора), тестирование с sandbox"

**Результат после discussion**: Consensus на **8 SP** (основная интеграция) + отдельная story **5 SP** (advanced error handling - продвинутая обработка ошибок).

> 💡 Расхождения в оценках выявляют скрытые assumptions (предположения) и риски.
...
**Story**: "Implement real-time notifications (реализовать real-time уведомления)"
...
**2 Stories в backlog**:
1. "Advanced analytics dashboard (продвинутая панель аналитики)" (для power users)
2. "One-click reorder (заказ в один клик)" (для busy moms like Sarah)
...
**Stories**:
- Story A: "Create order database schema (создать схему БД)"
...
*(Caching — это implementation detail (деталь реализации), не пользовательская ценность)*
...
**Результат**: Inconsistent quality (непостоянное качество), технический долг.
...
**Проблема**: Команда anchor'ится (привязывается) к этому числу, не думает критически.
...
### 8. Используйте Personas для empathy (эмпатии)

"As Sarah (busy mom)" лучше чем "As user" → думаем о реальных людях.

### 9. Definition of Done для consistency (постоянства)

Все stories проходят через одинаковые quality gates (ворота качества).
...
2. **INVEST criteria (критерии)**: Independent, Negotiable, Valuable, Estimable, Small, Testable
...
8. **Small & Focused (маленькие и сфокусированные)**: 1-5 SP, влезает в спринт, доставляет ценность
...
async function createUserStoriesLecture() {
  console.log('🚀 Создание лекции: User Stories (v2 с переводами)...\n');
  ...
  try {
    ...
    console.log(`📊 Объем: ${lectureContent.split('\n').length} строк`);
